...
class Imageable extends Behavior implements BehaviorInterface
{
    ...
    */
    ...
    protected function setTinipngService(array $options)
    {
        if (isset($options['tinipng']) && $options['tinipng'] == true) {
            $this->tinipng = true;
            \Tinify\setKey($this->config->global->tinipngKey);
        }
                    ...
                    if ($file->moveTo($fullPath)) {
                        ...
                        if ($this->tinipng == true) {
                            ...
                            try {
                                ...
                                $resizedMedium = $source->resize(array(
                                    "method" => "fit",
                                    "width" => 340,
                                    "height" => 434
                                ));

                                $resizedThumb = $source->resize(array(
                                    "method" => "fit",
                                    "width" => 190,
                                    "height" => 242
                                ));

                                $resizedMedium->toFile(PUBLIC_PATH . $mediumPath);
                                $resizedThumb->toFile(PUBLIC_PATH . $thumbPath);
